...
public class RecyclerViewFastScroller extends LinearLayout {
    ...
    private void setRecyclerViewPosition(float y) {
        if (recyclerView != null) {
            ...
            if (ViewCompat.getY(handle) == 0)
                proportion = 0f;
            else if (ViewCompat.getY(handle) + handle.getHeight() >= height - TRACK_SNAP_RANGE)
                proportion = 1f;
            else
    ...
    private void setBubbleAndHandlePosition(float y) {
        ...
        ViewCompat.setY(handle,getValueInRange(getPaddingTop(), height - handleHeight, (int) (y - handleHeight / 2)));

        if (bubble != null) {
            int bubbleHeight = bubble.getHeight();
            ViewCompat.setY(bubble,getValueInRange(getPaddingTop(), height - bubbleHeight - handleHeight / 2, (int) (y - bubbleHeight)));
